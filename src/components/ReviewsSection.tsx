    ...
    <section className="bg-[#F7F9FC] py-14">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            <h2 className="font-oswald text-2xl font-bold text-[#0D1B2A] mb-1">
              Отзывы клиентов
            </h2>
          ...
          {avg && (
            <div className="flex items-center gap-3 bg-white rounded-2xl px-5 py-3 shadow-sm border border-gray-100">
              ...
              <div>
                <div className="flex gap-0.5 mb-0.5">{stars(Math.round(Number(avg)))}</div>
                <p className="text-xs text-gray-400">{reviews.length} {reviews.length === 1 ? "отзыв" : reviews.length < 5 ? "отзыва" : "отзывов"}</p>
              </div>
            </div>
          )}
        </div>
